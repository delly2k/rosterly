...
function parseDutiesArray(raw: unknown): string[] {
...
function initialMapQuery(gig: Gig): string {
  return [gig.location_street, gig.location_city, gig.location_parish, "Jamaica"]
    .filter(Boolean)
    .join(", ");
}
...
export function EditGigForm({ gig }: { gig: Gig }) {
  ...
  const [duties, setDuties] = useState<string[]>(() => parseDutiesArray(gig.duties));
  const [dutyInput, setDutyInput] = useState("");
  const [dutiesError, setDutiesError] = useState<string | null>(null);
  const [selectedPayments, setSelectedPayments] = useState<string[]>(() => {
    try {
      return JSON.parse(gig.payment_method_dummy ?? "[]");
    } catch {
      return [];
    }
  });
  const [paymentOpen, setPaymentOpen] = useState(false);
  const [locationStreet, setLocationStreet] = useState(gig.location_street ?? "");
  const [locationCity, setLocationCity] = useState(gig.location_city ?? "");
  const [locationParish, setLocationParish] = useState(gig.location_parish ?? "");
  const [debouncedMapQuery, setDebouncedMapQuery] = useState(initialMapQuery(gig));
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      >
        ...
        >
          ...
          <div style={{ gridColumn: "1 / -1" }}>
            <label style={labelStyle}>Duties</label>
